...
export const OrderPage: React.FC = () => {
  const { cart, cartTotal, clearCart, showToast, navigateTo, addToCart } = useCart();
  ...
  const deliveryFee = deliveryType === 'delivery' ? (cartTotal >= 75 ? 0 : 5) : 0;
  const taxAmount = (cartTotal * 0.08);
  const finalTotal = cartTotal > 0 ? cartTotal + deliveryFee + taxAmount : selectedProduct.price * quantity + deliveryFee + (selectedProduct.price * quantity * 0.08);
      ...
      <main className="w-full bg-[#fbf4ea] text-[#222225] py-20 font-sans min-h-[70vh]">
        <div className="max-w-2xl mx-auto px-6 text-center space-y-6">
          ...
          <p className="text-sm text-[#666666] leading-relaxed max-w-lg mx-auto">
            Thank you <span className="font-bold text-[#222225]">{customerInfo.customerName}</span>! Your Cravie cheesecake order has been received and sent to our master baker.
          </p>
          <div className="bg-white p-6 rounded-3xl border border-[#ebd8c5] shadow-sm text-left text-xs space-y-3">
            <div className="flex justify-between border-b border-[#ebd8c5] pb-2">
              ...
              <span className="font-bold text-[#596613]">Freshly Baking 🍰</span>
            </div>
            ...
            <div className="flex justify-between">
              <span>Date & Time:</span>
              <span className="font-semibold">{customerInfo.deliveryDate} ({customerInfo.deliveryTime})</span>
            </div>
            {customerInfo.cakeMessage && (
              <div className="flex justify-between">
                <span>Message on Cake:</span>
                <span className="font-bold text-[#801818]">"{customerInfo.cakeMessage}"</span>
              </div>
            )}
      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-12">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-xs text-[#666666] max-w-lg mx-auto mt-2">
            Select your preferred flavor, size, custom message, and delivery slot. Freshly baked and delivered to your doorstep.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmitOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-[#ebd8c5] space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {PRODUCTS.slice(0, 4).map((item) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] font-bold text-[#801818]">${item.price}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {['Strawberry Glaze', 'Matcha Velvet', 'Dark Chocolate', 'Lemon Zest'].map((flv) => (
                    <button
                      type="button"
                      key={flv}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {['Single Slice', 'Half Cake (4)', 'Whole Cake (8)'].map((sz) => (
                    <button
                      type="button"
                      key={sz}
                      onClick={() => setSize(sz)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCustomerInfo({ ...customerInfo, cakeMessage: e.target.value })}
          ...
          <div className="lg:col-span-5 bg-white p-8 rounded-3xl shadow-sm border border-[#ebd8c5] space-y-6 sticky top-28">
            ...
            <div className="space-y-3">
              {cart.length > 0 ? (
                cart.map((item, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-[#fbf4ea] rounded-2xl border border-[#ebd8c5]">
                    <img src={item.product.images[0]} alt={item.product.name} className="w-12 h-12 object-cover rounded-xl" />
                    <div className="flex-1 min-w-0">
                      <p className="font-display font-extrabold text-xs text-[#222225] truncate">{item.product.name}</p>
                      <p className="text-[10px] text-[#666666]">Qty: {item.quantity} • {item.selectedFlavor || flavor}</p>
                    </div>
                    <span className="font-extrabold text-xs text-[#801818]">${(item.product.price * item.quantity).toFixed(2)}</span>
                  </div>
                ))
              ) : (
                <div className="flex items-center gap-3 p-3 bg-[#fbf4ea] rounded-2xl border border-[#ebd8c5]">
                  <img src={selectedProduct.images[0]} alt={selectedProduct.name} className="w-12 h-12 object-cover rounded-xl" />
                  <div className="flex-1 min-w-0">
                    <p className="font-display font-extrabold text-xs text-[#222225] truncate">{selectedProduct.name}</p>
                    <p className="text-[10px] text-[#666666]">Qty: {quantity} • {flavor}</p>
                  </div>
                  <span className="font-extrabold text-xs text-[#801818]">${(selectedProduct.price * quantity).toFixed(2)}</span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2 text-xs text-[#666666] pt-2 border-t border-[#ebd8c5]">
              <div className="flex justify-between">
                <span>Subtotal</span>
                <span>${(cartTotal || selectedProduct.price * quantity).toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
