...
export const ProductDetailPage: React.FC = () => {
  ...
  return (
    <main className="w-full bg-[#fbf4ea] text-[#222225] pt-4 sm:pt-6 pb-12 sm:pb-16 font-sans">
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 md:px-12">
        {/* Breadcrumb */}
        <nav className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-bold text-[#666666] uppercase tracking-wider mb-4 sm:mb-6">
          <a
            href="#"
            onClick={(e) => {
              e.preventDefault();
          ...
          <span>/</span>
          <span className="text-[#801818] truncate max-w-[200px] sm:max-w-none">{product.name}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col gap-4 sm:gap-6">
            <div className="relative w-full h-[280px] sm:h-[380px] md:h-[480px] lg:h-[500px] bg-white rounded-2xl sm:rounded-3xl overflow-hidden shadow-md border border-[#ebd8c5] p-3 sm:p-4 flex items-center justify-center">
              <img
                src={product.images[activeImageIndex] || product.images[0]}
                alt={product.name}
                className="w-full h-full object-cover rounded-xl sm:rounded-2xl"
              />
              <div className="absolute top-4 left-4 sm:top-6 sm:left-6 bg-[#801818] text-white text-[9px] sm:text-[10px] font-extrabold px-2.5 sm:px-3 py-1 rounded-full uppercase tracking-wider shadow-md">
                ★ CRAVIE SIGNATURE
              </div>
              <button
                onClick={() => toggleWishlist(product.id)}
                className={`absolute top-4 right-4 sm:top-6 sm:right-6 p-2 rounded-full shadow-md transition-colors ${
                  isWishlisted
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-4">
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`h-16 sm:h-20 md:h-24 rounded-xl sm:rounded-2xl overflow-hidden border-2 transition-all shadow-sm ${
            ...
            {/* Quality Badges */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#ebd8c5] shadow-sm">
          ...
          <div className="lg:col-span-5 bg-white p-5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-[#ebd8c5] space-y-4 sm:space-y-6">
            <div>
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#801818] block mb-1">
                CHEESECAKE SELECTION
              </span>
              <h1 className="font-display text-2xl sm:text-3xl font-extrabold text-[#222225] mb-2">
                {product.name}
              </h1>
              ...
              <div className="flex items-baseline gap-3">
                <span className="font-display text-2xl sm:text-3xl font-extrabold text-[#801818]">
                  ₹{product.price}
                </span>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`py-2 px-2.5 sm:px-3 rounded-xl text-[11px] sm:text-xs font-bold border-2 transition-all ${
                ...
                {["Single Slice", "Half Cake (4)", "Whole Cake (8)"].map(
                  (size) => (
                    <button
                      ...
                      className={`py-2 px-3 rounded-xl text-[11px] sm:text-xs font-bold border-2 transition-all flex-1 sm:flex-initial text-center ${
                        selectedSize === size
        ...
        <div className="mt-12 sm:mt-16 text-center">
          <h3 className="font-display text-xl sm:text-2xl font-extrabold text-[#801818] mb-6 uppercase">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            {relatedProducts.map((rel) => (
              <div
                ...
                className="bg-white rounded-2xl sm:rounded-3xl p-4 shadow-sm border border-[#ebd8c5] cursor-pointer hover:shadow-md transition-all group"
              >
                <div className="aspect-square rounded-xl sm:rounded-2xl overflow-hidden mb-3">
                  <img
                    src={rel.images[0]}
                    alt={rel.name}
